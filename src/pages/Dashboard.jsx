import { useState, useEffect } from "react"; // FIXED: Added useEffect to imports
...
export default function Dashboard() {
  ...
  // FIXED: Moved useEffect INSIDE the component
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [currentView]);
  ...
  // 2. Data Logic for Category Breakdown
  const expenseMap = {};
  ...
  // Helper for breakdown colors (Moved inside or kept as pure function below)
  function getCategoryColor(category) {
    const cat = category.toLowerCase();
    if (cat.includes("shop")) return "bg-pink-500";
    if (cat.includes("trav")) return "bg-orange-400";
    if (cat.includes("food") || cat.includes("dine")) return "bg-blue-500";
    if (cat.includes("rent") || cat.includes("home")) return "bg-teal-400";
    if (cat.includes("elec") || cat.includes("util")) return "bg-amber-400";
    if (cat.includes("game")) return "bg-sky-400";
    if (cat.includes("net") || cat.includes("wifi")) return "bg-emerald-400";
    if (cat.includes("movie") || cat.includes("ent")) return "bg-lime-400";
