...
export default function TransactionTable() {
  ...
  // 1. Add Date Filter States
  const [startDate, setStartDate] = useState("");
  const [endDate, setEndDate] = useState("");

  // 2. Updated Filter Logic to include Date Range
  ...
  return (
    <div className="space-y-4">
      {/* Table Header & Combined Filters */}
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-4 mb-2 px-1">
        <div>
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <div className="flex flex-col">
              ...
              <input 
                ...
                className="bg-gray-100 dark:bg-white/5 text-xs font-bold p-2 rounded-lg outline-none border border-transparent focus:border-emerald-500/50 dark:text-white transition-all"
              />
            </div>
            <div className="flex flex-col">
            ...
            {(startDate || endDate) && (
              <button 
                ...
                className="mt-4 text-[10px] font-bold text-rose-500 hover:text-rose-600 uppercase"
              >
                Clear
              </button>
            )}
          ...
          <div className="flex flex-col">
            <label className="text-[10px] font-black text-gray-400 uppercase ml-1">Sort By</label>
            <select 
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value)}
              className="bg-transparent text-sm font-bold text-emerald-600 dark:text-emerald-400 outline-none cursor-pointer h-9 px-1"
            >
          ...
          <table className="w-full text-left border-collapse bg-white dark:bg-white/5 backdrop-blur-md">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-white/5">
              {sortedTransactions.map((t) => (
                <tr key={t.id} className="hover:bg-indigo-50/30 dark:hover:bg-white/5 transition-colors duration-200 group">
